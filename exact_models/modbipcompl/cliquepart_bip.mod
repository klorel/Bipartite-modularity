...
#divisione in due
#subject to vincd{i in R, j in B, k in B: i<j<k}: t[i,j]+t[j,k]+t[i,k]>=2;
...
#altro
#subject to vinc4{i in V, j in V, k in V}: t[i,j]+t[j,k]+t[i,k]; somma puo' essere 0, 2, 3 -- guardo anche i segni dei coeff di t per decidere che vincoli usare 
