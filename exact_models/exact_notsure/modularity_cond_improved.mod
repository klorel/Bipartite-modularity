...
##subject to extraweak_cond {k in K, l in K : l!=k} : 2* sum{(u,v) in E} x[u,v,k] >=  sum{(u,v) in E} (2*z[u,v,k,l] - y[u,k] - y[v,l]) + 1;

## remark: constraint weak_cond does not allow to have empty classes (because of +1) Lo metto solo per i weak
